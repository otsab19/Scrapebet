...
class FootballRatings():
    ...
    def extract_vs(self, value_y, root):
        ...
        xpath = '//*[@class="blatt"]/svg//text[@x=200 and @y=' + \
            str(value_y) + ']/parent::a/@*'
        ...
        extra = html_sel.xpath(
            '//svg[@height=240 and @width=540]/g/text/text()')[:9]
        odds_drift = html_sel.xpath(
        ...
        if extra != []:
            ...
            try:
                if re.search(r'[\d]+', extra[0]).group() is not None:
    ...
    def extract_add(self, value_y, root):
        ...
        keys_rows = [
            ...
            'Domestic']
        for x in 20, 235:
            xpath = '//*[@class="blatt"]/svg//text[@x=' + \
                str(x) + ' and @y=' + str(value_y) + ']/parent::a/@*'
            url = self.sel.xpath(xpath)[0]
            url = 'http://clubelo.com' + url
            res = requests.get(url)
            html_sel = html.fromstring(res.content)

            data = [i for i in html_sel.xpath(
                '/html/body/div/div[9]/table/td/text()')]
            le = int(len(data) / 10)
            if x == 20:
    ...
    def jsontoxml(self, obj, value_y, date=None):
        ...
        pred = [ob[i] for ob in obj for i in ob.keys(
        ) if i == 'PredAH0Home' or i == 'PredAH0Draw' or i == 'PredAH0Away']
        if len(pred) == 3:
        ...
        country_home = [
            ob[i] for ob in obj for i in ob.keys() if i == 'country_image_home'][0]
        country_away = [
            ob[i] for ob in obj for i in ob.keys() if i == 'country_image_away'][0]
    ...
    def parse(self, response, date=None):
        ...
        self.sel = html_sel
        x = html_sel.xpath(
            '//*[@class="blatt"]/svg//text//@x[not(ancestor::g)]')
        y = html_sel.xpath(
            '//*[@class="blatt"]/svg//text//@y[not(ancestor::g)]')
        ...
        for i, j in zip(x, y):
            country = ''
            try:
                list_item = [item for item in mapp if (mapp[item].__class__ == list and (
                    mapp[item][0] == int(i) or mapp[item][1] == int(i))) or mapp[item] == int(i)]
                xpath = '//*[@class="blatt"]/svg//text[@x=' + \
                    i + 'and @y=' + j + ']/text()'
                if i == '20' or i == '235':
        ...
        if self.scrape_type == 'Results' or self.scrape_type == 'Fixtures':
            if self.date is not None:
                if self.end_date is not None:
                    ...
                    response = requests.get(
                        'http://clubelo.com/' + self.date + '/' + self.scrape_type)
                ...
                if self.end_date is not None:
                    for dt in rrule(
                        ...
                        until=datetime.datetime.strptime(
                            ...
                            "%Y-%m-%d").date()):
                        response = requests.get(
                            'http://api.clubelo.com/' + dt.strftime("%Y-%m-%d"))
                        self.parse_rank(response, dt.strftime("%Y-%m-%d"))
                else:
                    response = requests.get(
                        'http://api.clubelo.com/' + self.date)
                    self.parse_rank(response, self.date)

            else:
                urls = ['http://api.clubelo.com/' +
